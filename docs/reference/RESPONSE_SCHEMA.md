...
package、extension、response、capability 與 backend matrix 的版本關係，以及從舊 42-tool client 升級到目前 source inventory 的 migration 步驟，見 [`PROTOCOL_VERSIONING_AND_MIGRATION.md`](../concepts/PROTOCOL_VERSIONING_AND_MIGRATION.md)。

Camera 與 LiDAR artifact 已使用此欄位。共用 TTL、容量、分塊下載、hash、刪除與 cleanup 契約見 [`ARTIFACT_TRANSPORT.md`](../concepts/ARTIFACT_TRANSPORT.md)；Camera 契約見 [`CAMERA_RGB.md`](CAMERA_RGB.md) 與 [`CAMERA_OUTPUTS.md`](CAMERA_OUTPUTS.md)；LiDAR `.npz`、typed fields 與 per-field hash 契約見 [`LIDAR_POINT_CLOUD.md`](LIDAR_POINT_CLOUD.md)。
