...
本 reference 對應 `docs/ISAACSIM_MCP_6_0_1_IMPLEMENTATION_TASK.md` 已完成的 Phase 6。它提供後續 agent 的閱讀路由與發布安全邊界。實作細節以連結的文件、測試與 scripts 為準。

## 讀取流程

1. 先用下表把需求對應到 6.1～6.3。
2. 測試或 live 驗收先讀 `docs/LIVE_TEST_HARNESS.md`，確認 marker、scratch 身分與 cleanup 證據。
3. 能力報告先讀 tracked JSON 與產生器；逐 tool 的 pass 必須追到實際 verifier。
4. 安裝、client migration 或 release 分別讀 `INSTALLATION_WINDOWS.md`、`PROTOCOL_VERSIONING_AND_MIGRATION.md`、`RELEASE_GATE.md`。
...
|---|---|---|---|
| 6.1 | Phase 6 item 24 | pytest test pyramid、Windows/Unix launcher CI、exact scratch-stage guard、unique namespace cleanup | `pyproject.toml`、`tests/conftest.py`、`isaac_mcp/live_testing.py`、`docs/LIVE_TEST_HARNESS.md` |
| 6.2 | Phase 6 item 25 | 128 named tools source inventory、逐項 evidence/status、read-only live snapshot、tracked JSON | `scripts/generate_all_tools_report.py`、`docs/ALL_TOOLS_TEST_REPORT.md`、`docs/ALL_TOOLS_TEST_RESULTS.json` |
| 6.3 | Phase 6 item 26 | secret-free install、protocol/version migration、wheel/fresh-venv/repository release gate | `docs/INSTALLATION_WINDOWS.md`、`docs/PROTOCOL_VERSIONING_AND_MIGRATION.md`、`docs/RELEASE_GATE.md`、`scripts/release_gate.ps1` |
...
`scripts/generate_all_tools_report.py` 從 tool decorators 盤點 128 個唯一 names。`--live` 只做 bounded read-only runtime/catalog snapshot；`--check` 不寫入 tracked Markdown/JSON，適合 release gate。新增、刪除或改名 tool 時，inventory tests、逐項 evidence、README tool table、response docs、Changelog 與 migration 必須一起更新。
...
6. TCP 8766 read-only 128-tool matrix；
